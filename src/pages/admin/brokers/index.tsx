...
const AdminBrokersPage = () => {
          ...
          <TableBody>
            {filteredBrokers.map((broker) => (
              <TableRow key={broker._id}>
                ...
                <TableCell>{broker.companyName || "N/A"}</TableCell>
                <TableCell>
                  {broker.isVerified ? (
  ...
  <div className="w-full h-48 flex items-center justify-center bg-gray-200 rounded-md text-4xl font-bold text-gray-600">
    {selectedBroker.name?.charAt(0).toUpperCase() || "B"}
  </div>
)}
              </div>
              <div className="space-y-2">
                ...
                <div>
                  ...
                  <p className="font-medium">{selectedBroker.companyName || "N/A"}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Phone</p>
                  <p className="font-medium">{selectedBroker.phone || "N/A"}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Location</p>
                  <p className="font-medium">{selectedBroker.location || "N/A"}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Specialization</p>
                  <p className="font-medium">{selectedBroker.specialization || "N/A"}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Experience</p>
                  <p className="font-medium">{selectedBroker.experience || 0} yrs</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Sales</p>
                  <p className="font-medium">{selectedBroker.sales || 0}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Status</p>
                  <p className="font-medium">
                    {selectedBroker.isVerified ? "Verified" : "Unverified"}
                  </p>
                </div>
              </div>
            </div>
